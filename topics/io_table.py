...
class io_perf():
    def __init__(self, write_runtime, read_runtime):
        '''Calculate statistics about I/O performance'''

        self.write_runtime = write_runtime
        self.read_runtime = read_runtime
        
        self.io_proportion = 1- (self.write_runtime + self.read_runtime)
    ...
    def io_perf_table(self):
        ...
        perf_str = [r'$C_{\mathrm{I/O}} \approx$' + f'{self.io_proportion:.3f}']
        
        if self.io_proportion >= 0.8:
            data[0] = perf_str
        elif self.io_proportion < 0.8 and self.io_proportion >= 0.6:
            data[1] = perf_str
        else:
            data[2] = perf_str
